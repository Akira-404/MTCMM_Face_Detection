...
def align_data(image_path, imgae_size, margin, gpu_memory_faction):
    ...
    image_list = []  # 图片列表
    for image in temp_image_path:
        img = misc.imread(os.path.expanduser(image), mode='RGB')  # 这样读出来的图片格式为numpy类型，后面就不需要再转换了
        ...
        prewhitened = facenet.prewhiten(aligned)  # 预白化
